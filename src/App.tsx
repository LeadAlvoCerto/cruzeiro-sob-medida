...
const App: React.FC = () => {
  ...
  const processAnalysis = async (data: LeadData) => {
    ...
    try {
      ...
      console.log("✅ Resultado IA recebido (RAW):", result);
      const normalized: any = { ...result };

// Garantir strings (se vier objeto, converte em string legível)
for (const key of ['solIntro', 'tradeOffs', 'preferenceQuestion', 'conversionTrigger', 'fastActionBonus']) {
  const v = normalized?.[key];
  if (typeof v !== 'string') {
    normalized[key] = typeof v === 'undefined' ? '' : JSON.stringify(v);
  }
}

// Garantir recommendations como array
if (!Array.isArray(normalized.recommendations)) {
  normalized.recommendations = [];
}

// Garantir bonusStack sempre como array em cada rec
normalized.recommendations = normalized.recommendations.map((r: any) => ({
  ...r,
  bonusStack: Array.isArray(r?.bonusStack) ? r.bonusStack : [],
}));

// Substitui o result pelo normalizado
const safeResult = normalized as AIAnalysis;

console.log("✅ recommendations:", (result as any)?.recommendations);
